    ...
    async uploadFiles() {
        ...
        for (let i = 0; i < totalFiles; i++) {
            ...
            try {
                ...
                // Trigger analysis if analysis manager is available
                if (window.analysisManager && result.data) {
                    try {
                        console.log(`🔬 Triggering analysis for ${file.name}...`);
                        await window.analysisManager.triggerAnalysis(
                            result.data.id, 
                            result.publicUrl
                        );
                        console.log(`✅ Analysis triggered successfully for ${file.name}`);
                    } catch (analysisError) {
